...
function SatoriDchange() {
  const numb = dreamsList.getElementsByTagName("li").length;
  const request = document.getElementById("SATORID-change");
  let currentchange = parseInt(POINTS.innerHTML);
  let removePOINTS = 1000;
  ...
  if (currentchange < removePOINTS) {
    ...
    const data = {
      dream: "Transfer:" + userAccount.innerHTML + "SATORID" + "2",
    };
    ...
    // Add the dream value to the list
    dreams.push("Transfer:" + userAccount.innerHTML + "SATORID" + "2");
    appendNewDream("Transfer:" + userAccount.innerHTML + "SATORID" + "2");
...
function SatoriDPOINTS() {
  const numb = dreamsList.getElementsByTagName("li").length;
  const request = document.getElementById("SatoriD-change");
  let currentchange = parseInt(request.innerHTML);

  if (currentchange < 23) {
    userAlert.innerHTML = "23 SatoriD to send requests";
  } else {
    // Request dreams from the app's SQLite database
    fetch("/getDreams", {})
      .then((res) => res.json())
    ...
    const data = {
      dream: "Sent:" + userAccount.innerHTML + "SATORID" + currentchange,
    };
    ...
    dreams.push("Sent:" + userAccount.innerHTML + "SATORID" + currentchange);
    appendNewDream("Sent:" + userAccount.innerHTML + "SATORID" + currentchange);
    request.innerHTML = "0";
    userAlert.innerHTML = "Request Sent!";
    pullENERGY();
    createDreamblock();
